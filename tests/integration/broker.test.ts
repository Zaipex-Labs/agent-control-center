...
describe('broker integration', () => {
  ...
  it('full message flow: register → send → poll → history', async () => {
    ...
    const regA = await post<{ id: string }>('/register', {
    ...
    const regB = await post<{ id: string }>('/register', {
      pid: process.pid, cwd: '/app/frontend', role: 'frontend', project_id: 'integ',
    });
    const idB = regB.data.id;

    // List peers — should see both
    const peers = await post<Array<{ id: string; role: string }>>('/list-peers', {
      project_id: 'integ',
    });
    expect(peers.data).toHaveLength(2);

    // A sends message to B
    const send1 = await post<{ ok: boolean }>('/send-message', {
    ...
    const send2 = await post<{ ok: boolean }>('/send-message', {
    ...
    const poll = await post<{ messages: Array<{ text: string; type: string }> }>('/poll-messages', { id: idB });
    ...
    const poll2 = await post<{ messages: unknown[] }>('/poll-messages', { id: idB });
    expect(poll2.data.messages).toHaveLength(0);

    // History shows both messages
    const history = await post<{ messages: Array<{ from_role: string; to_role: string }> }>('/get-history', {
      project_id: 'integ',
    });
  ...
  it('shared state round-trip', async () => {
    ...
    // Get missing
    const missing = await post<{ error: string }>('/shared/get', {
  ...
  it('heartbeat updates last_seen', async () => {
    ...
    const hb = await post<{ ok: boolean }>('/heartbeat', { id: reg.data.id });
  ...
  it('unregister removes a peer', async () => {
    ...
    await post('/unregister', { id: reg.data.id });

    // Heartbeat should now fail
    const hb = await post<{ ok: boolean; error?: string }>('/heartbeat', { id: reg.data.id });
