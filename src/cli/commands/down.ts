  ...
  program
    ...
    .action(async (projectName: string) => {
      ...
      for (const peer of peers) {
        ...
        if (wasKilled) {
          console.log(`  ${success('Stopped')} ${chalk.magenta(peer.role || peer.id)} ${dim(`(pid: ${peer.pid})`)}`);
          killed++;
        } else {
          console.log(`  ${dim('Already dead')} ${chalk.magenta(peer.role || peer.id)} ${dim(`(pid: ${peer.pid})`)}`);
        }
      ...
      if (hasTmuxSession(projectName)) {
        ...
        if (sessionKilled) {
          console.log(`  ${success('Killed')} tmux session ${dim(`acc-${projectName}`)}`);
        }
      }

      if (peers.length === 0 && !hasTmuxSession(projectName)) {
        console.log(warn(`  No active agents found for project "${projectName}".`));
      } else {
        console.log(dim(`\n  Stopped ${killed} agent(s) for "${projectName}".`));
        console.log(dim(`  Broker left running (history and shared state preserved).`));
      }
    });
}
